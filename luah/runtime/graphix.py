...
_yurifont_lock  = threading.Lock()

def _yuriget_font(size):

    global _yurifont_cache

    if len(_yurifont_cache) > 64:

        _yurifont_cache.clear()

    with _yurifont_lock:

        if size not in _yurifont_cache:

            _yurifont_cache[size] = pygame.font.SysFont("monospace", size)

        return _yurifont_cache[size]

class GraphixWindow:

    def __init__(self, yuriwidth, yuriheight, yurititle, yuristopevent,

                 yurifullscreen=False, yuriborderless=False):
        ...
        self._fullscreen   = yurifullscreen

        self._borderless   = yuriborderless
        ...
        self._pending_cbs  = queue.Queue(maxsize=500)                    
    ...
    def get_lua_table(self, yurilua):
        ...
        def set_fullscreen(_s, yuristate=True):

            yuricmd("set_fullscreen", bool(yuristate))

        def set_borderless(_s, yuristate=True):

            yuricmd("set_borderless", bool(yuristate))
        ...
        def fill_rect(_s, x, y, w, h, r, g, b):
            ...
            yuricmd("fill_rect", int(x), int(y), w, h,
        ...
        def draw_rect(_s, x, y, w, h, r, g, b, lw=1):
            ...
            yuricmd("draw_rect", int(x), int(y), w, h,
        ...
        def draw_line(_s, x1, y1, x2, y2, r, g, b, lw=1):
            ...
            yuricmd("draw_line", int(x1), int(y1), int(x2), int(y2),
                    ...
                    yuriclamp(r), yuriclamp(g), yuriclamp(b), max(1, int(lw)))
        ...
        def fill_round_rect(_s, x, y, w, h, radius, r, g, b):
            ...
            yuricmd("fill_round_rect", int(x), int(y), w, h,
        ...
        def draw_round_rect(_s, x, y, w, h, radius, r, g, b, lw=1):
            ...
            yuricmd("draw_round_rect", int(x), int(y), w, h,
        ...
        def get_text_width(_s, text, size=16):
            ...
            return _yuriget_font(max(6, int(size))).size(str(text))[0]

        def get_text_height(_s, size=16):

            return _yuriget_font(max(6, int(size))).get_height()
        ...
        def run(_s, fps=60):
            ...
            while not yuriw._closed.is_set() and not yuriw._stop.is_set():
                ...
                yuriw._frame_done.wait(timeout=0.05)
    ...
    def _pygame_thread(self):
        ...
        if self._fullscreen:
            ...
            yuriscreen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)

            self.width  = yuriscreen.get_width()

            self.height = yuriscreen.get_height()

        elif self._borderless:

            if self.width > 0 and self.height > 0:

                yuriscreen = pygame.display.set_mode((self.width, self.height), pygame.NOFRAME)

            else:

                yuriscreen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)

                yuridw = yuriscreen.get_width()

                yuridh = yuriscreen.get_height()

                yuriscreen = pygame.display.set_mode((yuridw, yuridh), pygame.NOFRAME)
    ...
    def _handle_cmd(self, screen, cmd, args, images):
        ...
        if cmd == "quit":
        ...
        elif cmd == "set_fullscreen":

            self._fullscreen = bool(args[0])
        ...
        elif cmd == "set_borderless":                                                  

            self._borderless = bool(args[0])

            yuriflags = pygame.NOFRAME if self._borderless else 0

            screen = pygame.display.set_mode((self.width, self.height), yuriflags)
            ...
            if a >= 255:
                ...
                pygame.draw.rect(s, (r, g, b, a), (0, 0, w, h), lw)
            ...
            if a >= 255:
                ...
                s = pygame.Surface((rad*2+1, rad*2+1), pygame.SRCALPHA)

                pygame.draw.circle(s, (r, g, b, a), (rad, rad), rad)

                screen.blit(s, (x-rad, y-rad))
            ...
            if a >= 255:
                ...
                s = pygame.Surface((rad*2+lw+1, rad*2+lw+1), pygame.SRCALPHA)

                pygame.draw.circle(s, (r, g, b, a), (rad+lw//2, rad+lw//2), rad, lw)

                screen.blit(s, (x-rad, y-rad))

        elif cmd == "draw_line":

            x1, y1, x2, y2, r, g, b, lw = args
            ...
            if a >= 255:
                ...
                pygame.draw.rect(s, (r, g, b, a), (0, 0, w, h), border_radius=radius)
            ...
            if a >= 255:
                ...
                pygame.draw.rect(s, (r, g, b, a), (0, 0, w, h), lw, border_radius=radius)
            ...
            if a >= 255:
                ...
                pygame.draw.polygon(s, (r, g, b, a), pts)
            ...
            if a >= 255:
                ...
                pygame.draw.polygon(s, (r, g, b, a), pts, lw)
...
class GraphixManager:
    ...
    def create_window(self, w, h, title, stop, fullscreen=False, borderless=False):

        win = GraphixWindow(w, h, title, stop, fullscreen, borderless)
        ...
        with self._lock:
            ...
            for win in self._wins:
                ...
                try:
                    ...
                    win._cmd_queue.put_nowait(("quit", ()))
