...
yuricompletions = {
    ...
    "graphix": ["newWindow(w, h, title, fullscreen, borderless)"],
    ...
    "win": [
        ...
        "onKeyDown(function(key, char) end)",

        "onKeyUp(function(key) end)",

        "onMouseDown(function(x, y, btn) end)",

        "onMouseUp(function(x, y, btn) end)",

        "onScroll(function(delta) end)",
    ...
    "math": [
        ...
        "sign(v)", "round(v)", "dist(x1, y1, x2, y2)", "uuid()",

        "pow(x, y)", "log(x)", "exp(x)", "fmod(x, y)", "modf(x)",
    ...
    "string": [
        ...
        "split(str, sep)", "trim(str)", "startsWith(str, prefix)",

        "endsWith(str, suffix)", "contains(str, sub)",

        "padLeft(str, len, char)", "padRight(str, len, char)",
    ...
    "table": [
        ...
        "map(t, fn)", "filter(t, fn)", "reduce(t, fn, init)",

        "contains(t, val)", "keys(t)", "values(t)", "clone(t)", "merge(a, b)",

    ],

    "time": ["now()", "clock()", "sleep(seconds)", "stamp()"],

    "luah": ["version", "platform", "arch", "python"],
    ...
    "http": [
        ...
        "serve(port, function(req) end)",

        "response(body, status, type)",
